...
async function probeBackend() {
  if (BACKEND_ALIVE !== null) return BACKEND_ALIVE;
  try {
    const ctrl = new AbortController();
    const tid = setTimeout(() => ctrl.abort(), 2000);
    await fetch(API + '/auth/me', { signal: ctrl.signal });
    clearTimeout(tid);
    BACKEND_ALIVE = true;
  } catch (_) {
    BACKEND_ALIVE = false;
...
function demoUsers() { return lsGet('cm_demo_users') || {}; }
function saveDemoUsers(u) { lsSet('cm_demo_users', u); }

function makeDemoToken(userId) { return 'demo_' + userId + '_' + Date.now(); }
function parseDemoToken(tok) {
  if (!tok || !tok.startsWith('demo_')) return null;
...
async function demoApiFetch(path, options = {}) {
  ...
  if (path === '/auth/register' && options.method === 'POST') {
    ...
    const user = { user_id: userId, name: body.name, email: body.email,
                   password: body.password, profiles: [] };
    users[userId] = user;
    saveDemoUsers(users);
    const token = makeDemoToken(userId);
  ...
  if (path === '/auth/login' && options.method === 'POST') {
    ...
    const token = makeDemoToken(user.user_id);
  ...
  if (path === '/profiles' && options.method === 'POST') {
    ...
    if (user.profiles.length >= 5) throw new Error('Maximum 5 profiles allowed');
    const profile = { profile_id: 'p_' + Math.random().toString(36).slice(2),
      name: body.name, avatar: body.avatar, gradient: body.gradient,
      is_kids: body.is_kids || false, taste: null, watch_history: [],
  ...
  if (path.startsWith('/profiles/') && options.method === 'PUT') {
    ...
    const user = users[userId];
    if (!user) throw new Error('Not authenticated');
    const p = user.profiles.find(x => x.profile_id === profileId);
    if (!p) throw new Error('Profile not found');
    if (body.taste !== undefined) p.taste = body.taste;
    if (body.watch_history !== undefined) p.watch_history = body.watch_history;
    saveDemoUsers(users);
  ...
  if (path.startsWith('/profiles/') && options.method === 'DELETE') {
    ...
    const user = users[userId];
    if (!user) throw new Error('Not authenticated');
    user.profiles = user.profiles.filter(x => x.profile_id !== profileId);
    saveDemoUsers(users);
  ...
  if (path === '/watched' && options.method === 'POST') {
    ...
    const user = users[userId];
    if (user) {
      const p = user.profiles.find(x => x.profile_id === body.profile_id);
      if (p && !p.watch_history.includes(body.tmdb_id)) p.watch_history.push(body.tmdb_id);
      saveDemoUsers(users);
    }
    return { status: 'saved' };
  }
...
function showDemoBanner() {
  ...
  banner.style.cssText = 'position:fixed;bottom:16px;left:50%;transform:translateX(-50%);z-index:9999;background:rgba(30,30,30,0.95);border:1px solid rgba(229,9,20,0.4);border-radius:12px;padding:10px 18px;font-size:12px;color:#aaa;display:flex;align-items:center;gap:8px;backdrop-filter:blur(12px);box-shadow:0 4px 24px rgba(0,0,0,0.5);white-space:nowrap';
  banner.innerHTML = '<span style="width:7px;height:7px;border-radius:50%;background:#f59e0b;flex-shrink:0"></span><span><strong style="color:#fff">Demo Mode</strong> — Backend offline. Data saved in browser.</span><button onclick="this.parentElement.remove()" style="background:none;border:none;color:#666;cursor:pointer;padding:0;font-size:16px;line-height:1;margin-left:8px">×</button>';
...
async function initApp() {
  if (state.token) {
    try {
      ...
      if (state.token && state.token.startsWith('demo_')) showDemoBanner();
...
function renderLogin() {
  ...
  $('#login-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.token && data.token.startsWith('demo_')) showDemoBanner();
      renderProfileSelection();
    } catch(e) {
